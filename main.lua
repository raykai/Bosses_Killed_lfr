...
function addon:GetButtonScale(numDungeons)
	...
	-- Mite need min 480 still too see
	return min(480 / (numDungeons * 32), 1)
end
...
function addon:UpdateButtonsAndTooltips(parentFrame)
	...
	for id, button in pairs(buttons) do
		...
		-- GetLFGDungeonNumEncounters returns the full set of bosses for the whole raid
...
function addon.UpdateArrow()
	...
	local parent
	if RaidFinderQueueFrame:IsVisible() then
